...
public section.
  ...
  data GT_SY_TESTS_IN type ZNEPT_QZ_SY_TESTS_IN_T .
  data GT_SY_TESTS_OUT type ZNEPT_QZ_SY_TESTS_OUT_T .
  ...
  data GT_SY_CHECK type ZNEPT_QZ_SY_CHECK_T .
  data GS_SY_TESTS_IN type ZNEPT_QZ_SY_TESTS_IN_S .
  data GS_SY_TESTS_OUT type ZNEPT_QZ_SY_TESTS_OUT_S .
  ...
  data GS_SY_CHECK type ZNEPT_QZ_SY_CHECK_S .
...
private section.
  ...
  constants GC_SY_ACT_UPDATE type CHAR1 value 'U' ##NO_TEXT.
  constants GC_SY_ACT_DELETE type CHAR1 value 'D' ##NO_TEXT.
  constants GC_SY_ACT_INSERT type CHAR1 value 'I' ##NO_TEXT.
...
* ------------------------------------------------------------
  ...
  METHOD ajax_refresh.
    ...
    DATA: ls_db_tests_key TYPE znept_qz_db_tests_key_s,
          ls_sy_tests_out TYPE znept_qz_sy_tests_out_s,
          ls_sy_tests_in  TYPE znept_qz_sy_tests_in_s,
          ...
          lt_sy_parts     TYPE znept_qz_sy_parts_t,
          lt_sy_questions TYPE znept_qz_sy_questions_t,
          lt_sy_variants  TYPE znept_qz_sy_variants_t,
          lt_sy_check     TYPE znept_qz_sy_check_t.

    FIELD-SYMBOLS: <fs_sy_tests_out> TYPE znept_qz_sy_tests_out_s.

    REFRESH: gt_sy_tests_out[], gt_sy_parts[], gt_sy_questions[], gt_sy_variants[], gt_sy_check[].

    LOOP AT gt_sy_tests_in INTO ls_sy_tests_in.

      MOVE-CORRESPONDING ls_sy_tests_in TO ls_db_tests_key.

      CLEAR ls_sy_tests_out.
      MOVE-CORRESPONDING ls_db_tests_key TO ls_sy_tests_out.

      CALL METHOD zcl_nept_qz_data_provider=>get
        EXPORTING
          is_db_tests_key = ls_db_tests_key
          iv_version      = ls_sy_tests_in-version
        IMPORTING
          ev_version      = ls_sy_tests_out-version
      ...
      MOVE-CORRESPONDING lt_db_parts[] TO lt_sy_parts[].
      MOVE-CORRESPONDING lt_db_questions[] TO lt_sy_questions[].
      MOVE-CORRESPONDING lt_db_variants[] TO lt_sy_variants[].
      MOVE-CORRESPONDING lt_db_check[] TO lt_sy_check[].

      APPEND LINES OF lt_sy_parts[] TO gt_sy_parts[].
      APPEND LINES OF lt_sy_questions[] TO gt_sy_questions[].
      APPEND LINES OF lt_sy_variants[] TO gt_sy_variants[].
      APPEND LINES OF lt_sy_check[] TO gt_sy_check[].

      APPEND ls_sy_tests_out TO gt_sy_tests_out.
    ENDLOOP.

* check if we selected new content to download then we are in "CHORT SYNC"
* we do not update the header

    IF gv_db_error IS INITIAL.

* or if we need to refresh all data then we are in "SYNC ALL"
...
* refreshing header
      ...
      LOOP AT lt_db_tests INTO ls_db_tests.
        ...
        READ TABLE gt_sy_tests_out ASSIGNING <fs_sy_tests_out> WITH KEY test_id   = ls_db_tests_key-test_id
                                                                        upload_on = ls_db_tests_key-upload_on
                                                                        upload_at = ls_db_tests_key-upload_at.
        IF sy-subrc <> 0.

* add if any new content

          APPEND INITIAL LINE TO gt_sy_tests_out ASSIGNING <fs_sy_tests_out>.
          MOVE-CORRESPONDING ls_db_tests_key TO <fs_sy_tests_out>.
          CLEAR <fs_sy_tests_out>-version.
        ENDIF.

* submit the latest header info

        <fs_sy_tests_out>-description       = ls_db_tests-description.
        <fs_sy_tests_out>-published         = ls_db_tests-published.
        <fs_sy_tests_out>-upload_by         = ls_db_tests-upload_by.
        <fs_sy_tests_out>-sy_upload_by_name = get_user_name( iv_user_id = ls_db_tests-upload_by ).
        zcl_nept_qz_data_provider=>info( EXPORTING is_db_tests_key    = ls_db_tests_key
                                         IMPORTING ev_total_parts     = <fs_sy_tests_out>-sy_count_parts
                                                   ev_total_questions = <fs_sy_tests_out>-sy_count_questions ).
      ENDLOOP.

* check for deleted / private content

      LOOP AT gt_sy_tests_out INTO ls_sy_tests_out.
        READ TABLE lt_db_tests TRANSPORTING NO FIELDS WITH KEY test_id   = ls_sy_tests_out-test_id
                                                               upload_on = ls_sy_tests_out-upload_on
                                                               upload_at = ls_sy_tests_out-upload_at.
        IF sy-subrc <> 0.
          DELETE gt_sy_tests_out.
        ENDIF.
      ENDLOOP.
...
* if there is only one record and SYNC_ID is initial then we are in "SHORT SYNC"

      READ TABLE gt_sy_sync_metrics INDEX 1 INTO ls_sy_sync.
      IF sy-subrc = 0 AND NOT ls_sy_sync-sync_id IS INITIAL.

* there might me more than one entity to sync then we are in "SYNC ALL"
* select all available metrics to the device
...
* sync Bookmarks
  ...
  METHOD ajax_download.
    ...
    CLEAR gs_sy_tests_out.

    REFRESH: gt_sy_parts[], gt_sy_questions[], gt_sy_variants[].

    MOVE-CORRESPONDING gs_sy_tests_in TO ls_db_tests_key.
    ...
    IF gv_db_error IS INITIAL.
      ...
      gs_sy_tests_out = ls_sy_tests_out.
    ENDIF.
